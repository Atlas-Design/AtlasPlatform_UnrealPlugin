...
/**
 * Information about a job's archive folder paths.
 * Canonical disk layout:
 * {OutputFolder}/{WorkflowName}/{JobFolderName}/job.json
 * {OutputFolder}/{WorkflowName}/{JobFolderName}/inputs/
 * {OutputFolder}/{WorkflowName}/{JobFolderName}/outputs/
 */
USTRUCT(BlueprintType)
struct ATLASSDK_API FAtlasJobFolderInfo
{
	...
	/** Full disk path for this job's run archive root */
	UPROPERTY(BlueprintReadOnly, Category = "Atlas|Output")
	FString DiskPath;

	/** Full disk path for archived input files used by this run */
	UPROPERTY(BlueprintReadOnly, Category = "Atlas|Output")
	FString InputsDiskPath;

	/** Full disk path for output files generated by this run */
	UPROPERTY(BlueprintReadOnly, Category = "Atlas|Output")
	FString OutputsDiskPath;

	/** Full disk path for the per-run metadata file */
	UPROPERTY(BlueprintReadOnly, Category = "Atlas|Output")
	FString JobJsonPath;
...
public:
	...
	/**
	 * Save audio bytes to the Audio subfolder.
	 * Convenience method that always uses the Audio/ subfolder regardless of auto-organize setting.
	 * @param Bytes The audio file content
	 * @param FileName The filename (with extension, e.g., "sound.mp3")
	 * @return Save result with path and success status
	 */
	UFUNCTION(BlueprintCallable, Category = "Atlas|Output")
	FAtlasSaveResult SaveAudioToOutputFolder(const TArray<uint8>& Bytes, const FString& FileName);

	// ==================== Job-Based Output Saving ====================

	/**
	 * Save file bytes to the job-specific outputs folder on disk.
	 * Compatibility alias for SaveToJobOutputFolder.
	...
	/**
	 * Save file bytes to the job-specific outputs folder on disk.
	 * Creates folder structure: {OutputFolder}/{WorkflowName}/{JobFolder}/outputs/{FileName}
	 * @param Job The job that produced this output
	 * @param Bytes The file content
	 * @param FileName The filename (with extension)
	 * @return Save result with path and success status
	 */
	UFUNCTION(BlueprintCallable, Category = "Atlas|Output")
	FAtlasSaveResult SaveToJobOutputFolder(UAtlasJob* Job, const TArray<uint8>& Bytes, const FString& FileName);
	...
	/**
	 * Get the Audio subfolder path (absolute).
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetAudioFolder() const;
	...
	/**
	 * Check if a filename represents an audio file based on extension.
	 * Supports: .mp3, .wav, .ogg, .flac
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	static bool IsAudioFile(const FString& FileName);
	...
	/**
	 * Sanitize a parameter or file label for use in generated filenames.
	 * @param ParameterName The original parameter name
	 * @return Sanitized name safe for filesystem use
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	...
	/**
	 * Get the run archive root folder for a job.
	 * Pattern: {OutputFolder}/{WorkflowName}/{JobFolder}/
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobRunFolder(UAtlasJob* Job) const;

	/**
	 * Get the archived inputs folder for a job.
	 * Pattern: {OutputFolder}/{WorkflowName}/{JobFolder}/inputs/
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobInputsFolder(UAtlasJob* Job) const;

	/**
	 * Get the generated outputs folder for a job.
	 * Pattern: {OutputFolder}/{WorkflowName}/{JobFolder}/outputs/
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobOutputsFolder(UAtlasJob* Job) const;

	/**
	 * Get the per-run metadata JSON path for a job.
	 * Pattern: {OutputFolder}/{WorkflowName}/{JobFolder}/job.json
	 */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobJsonPath(UAtlasJob* Job) const;

	/** Get the run archive root folder from a history record. */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobRunFolderFromHistory(const FAtlasJobHistoryRecord& HistoryRecord) const;

	/** Get the archived inputs folder from a history record. */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobInputsFolderFromHistory(const FAtlasJobHistoryRecord& HistoryRecord) const;

	/** Get the generated outputs folder from a history record. */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobOutputsFolderFromHistory(const FAtlasJobHistoryRecord& HistoryRecord) const;

	/** Get the per-run metadata JSON path from a history record. */
	UFUNCTION(BlueprintPure, Category = "Atlas|Output")
	FString GetJobJsonPathFromHistory(const FAtlasJobHistoryRecord& HistoryRecord) const;

	/**
	 * Get complete folder information for a job.
	 * Creates the disk structure: {OutputFolder}/{WorkflowName}/{JobFolder}/
